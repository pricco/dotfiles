...
if [[ $SHOW_DIRECTORY_NAME -eq 1 ]]; then
    set "window-status-format" "#[fg=$onedark_white,bg=$onedark_black] #(basename #{pane_current_path})  "
    set "window-status-current-format" "#[fg=$onedark_blue,bg=$onedark_black,nobold,nounderscore,noitalics]#[fg=$onedark_black,bg=$onedark_blue,nobold] #(basename #{pane_current_path}) / #W #[fg=$onedark_blue,bg=$onedark_black,nobold,nounderscore,noitalics]"
else
    set "window-status-format" "#[fg=$onedark_white,bg=$onedark_black] #W "
    set "window-status-current-format" "#[fg=$onedark_blue,bg=$onedark_black,nobold,nounderscore,noitalics]#[fg=$onedark_black,bg=$onedark_blue,nobold] #W #[fg=$onedark_blue,bg=$onedark_black,nobold,nounderscore,noitalics]"
fi
